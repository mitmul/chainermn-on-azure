...
def delete_vm(name):
    ...
    public_ip_name = nic_info['ipConfigurations'][0]['publicIpAddress']['id'].split('/')[-1]
    ...
    run("az network public-ip delete -g {resource_group} -n {public_ip_name}".format(
        resource_group=RESOURCE_GROUP, public_ip_name=public_ip_name))
