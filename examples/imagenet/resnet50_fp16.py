...
class BottleNeckB_fp16(chainer.Chain):

    def __init__(self, in_size, ch):
        self.dtype = dtype = np.float16
        W = initializers.HeNormal(1 / np.sqrt(2), self.dtype)

        super(BottleNeckB_fp16, self).__init__()
        with self.init_scope():
            self.conv1 = L.Convolution2D(
...
class Block_fp16(chainer.ChainList):

    def __init__(self, layer, in_size, ch, out_size, stride=2):
        super(Block_fp16, self).__init__()
        self.add_link(BottleNeckA_fp16(in_size, ch, out_size, stride))
        for i in range(layer - 1):
            self.add_link(BottleNeckB_fp16(out_size, ch))
...
class ResNet50_fp16(chainer.Chain):
    ...
    def __init__(self):
        ...
        super(ResNet50_fp16, self).__init__()
        with self.init_scope():
            self.conv1 = L.Convolution2D(
                3, 64, 7, 2, 3, initialW=W, initial_bias=bias)
            self.bn1 = L.BatchNormalization(64, dtype=dtype)
            self.res2 = Block_fp16(3, 64, 64, 256, 1)
            self.res3 = Block_fp16(4, 256, 128, 512)
            self.res4 = Block_fp16(6, 512, 256, 1024)
            self.res5 = Block_fp16(3, 1024, 512, 2048)
